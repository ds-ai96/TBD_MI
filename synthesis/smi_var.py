...
class SMI_VAR(BaseSynthesis):
    def __init__(self, teacher,teacher_name, student, num_classes, img_shape=(3, 224, 224),patch_size=16,
                 ...
                 bnsource='resnet50v2',init_dataset=None, variance=-1):
        super(SMI_VAR, self).__init__(teacher, student)
    ...
    def synthesize(self, targets=None,num_patches=197,prune_it=[-1],prune_ratio=[0]):
        ...
        self.teacher.eval()
        best_cost = 1e6
        inputs = torch.randn( size=[self.synthesis_batch_size, *self.img_size], device=self.device) * self.variance
