...
if __name__ == '__main__':
    ...
    evaluate_only=False
    teacher_name = 'deit_tiny_16_cifar10'
    pathc_size=16 if '16' in teacher_name else 32
    num_patch=197 if pathc_size==16 else 50
    if evaluate_only==False:
        ...
        if 'swin'in teacher_name:
            ...
            batch_size=256
        dataset_output=build_dataset(teacher_name.split('_')[0],teacher_name.split('_')[-1],batch_size,train_aug=True,keep_zero=True,train_inverse=False)
        ...
        model = model.to(device)
        model.eval()
        dataset_output = build_dataset(teacher_name.split('_')[0], teacher_name.split('_')[-1], 256, train_aug=True,
                                       keep_zero=True, train_inverse=False)
        train_loader, val_loader = dataset_output[0], dataset_output[1]
